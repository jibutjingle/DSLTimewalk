...
Shader "Reallusion/Amplify/RL_CorneaShaderParallax_URP_Dissolve"
{
	Properties
	{
		...
		_NoiseTex("Dissolve Noise", 2D) = "Assets/Shaders/DissolveShaders/noiseTexs/NoiseEffectGrain.png" {}
		_DissolveAmount("Dissolve Amount", Range(0, 1)) = 0
		[HDR]_DissolveColor("Dissolve Edge Color", Color) = (1.741, 1.614, 1.092, 0)
		_DissolveWidth("Dissolve Edge Width", Range(0, 0.2)) = 0.01
	...
	SubShader
	{
		...
		Pass
		{
						 ...
						 ) : SV_Target
			{
				...
				float dissolveNoise = SAMPLE_TEXTURE2D(_NoiseTex, sampler_NoiseTex, uv_NoiseTex).r;
		...
		Pass
		{
			...
			struct VertexOutput
			{
				...
				#endif
				float2 ase_dissolveUV : TEXCOORD3;				
			...
			VertexOutput VertexFunction( VertexInput v )
			{
				...
				o.ase_dissolveUV = v.texcoord.xy;
				return o;
			}
						 ...
						 ) : SV_TARGET
			{
				...
				float2 uv_NoiseTex = IN.ase_dissolveUV * _NoiseTex_ST.xy + _NoiseTex_ST.zw;
				float dissolveNoise = SAMPLE_TEXTURE2D(_NoiseTex, sampler_NoiseTex, uv_NoiseTex).r;
				clip(dissolveNoise - _DissolveAmount);
	...
	CustomEditor "Reallusion.Import.DissolveAmplifyCharacterShaderGUI"
